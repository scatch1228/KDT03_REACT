...
export default function TodoItem({ todo }) {
    ...
    //page load
    useEffect(()=>{
      console.log('[TodoItem] localTodo = ',localTodo);

      setTodos(localTodo);
      setComp(localTodo.filter(item=>item.completed == true));
    },[])

    //on todos change / on all handleXX methods
    useEffect(()=>{
      if (!todos) return ;
      console.log('[TodoItem] state todos = ',todos);
      
    },[todos])

    const handleToggle = ()=>{
        //localStorage.setItem('todo',JSON.stringify(todos.map(t=>t.id == todo.id ? {...t, completed : !todo.completed} : t)))
        setTodos(
            ...
            <div className="w-8/10 flex flex-row">
                <input type="checkbox" checked={todo.completed} onChange={handleToggle} className="mx-5" />
                {
                    ...
                    />
                        ...
                        : <span>{todos.filter(t => t.id == todo.id)[0]  }</span> //!todos ? "null" : 
                }
            </div>
